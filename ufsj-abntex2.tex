...
A geração de código é feita por meio da interpolação e substituição de \textit{strings} em \textit{templates}. Estes \textit{templates} incluem o código base necessário para a simulação e plotagem com as bibliotecas corretas inclusas, ecritos de maneira sucinta e legível com o objetivo de servir como um material de aprendizagem estensível.

Em \ref{code:jinja-template}, observa-se parte do template que extrai o valor de variáveis e constantes para serem usados na simulação do próximo passo de tempo. Neste trecho, nota-se o uso de estruturas de controle como condicionais (\texttt{if}) e laços de repetição (\texttt{for}). Baseando a geração de código no sistema de \textit{templates}, é possível adicionar novas formas de simulações sem alterar o código do programa. Por exemplo, é possível adicionar um \textit{template} para gerar o código do modelo que o simulará baseado no método de Gillespie.
...
Em \ref{code:ext-sin-pow} é apresentado um exemplo de arquivo de extensões contendo um nó para calcular o seno de um parâmetro e um nó que eleva um parâmetro ao valor de outro. Este arquivo pode ser importado na interface, que possibilitará o uso dos nós como na figura \ref{fig:ext-sin-pow}.

\begin{lstlisting}[language=Python, label=code:ext-sin-pow, caption=Exemplo de definição dos nós de extensão seno e potência com código Python válido.][H]
import math
...
O software desenvolvido foi dividido em dois módulos: (1) o responsável pela interface gráfica e (2) o responsável pela representação intermediária. Este primeiro lida com a apresentação de gráficos, interações com o usuário e a plotagem dos resultados, enquanto o segundo define a representação em JSON dos modelos e realiza a transformação em código de Python.
...
O módulo de GUI [\ref{sub:gui}] foi inicialmente desenvolvido em C++. Entretanto, rapidamente se tournou desafiador manter uma grande base de código livre de \textit{bugs} relacionados a ponteiros nulos e liberações duplas (\textit{double frees}). Esta mesma dificuldade foi ressaltada pela agência de segurança nacional dos EUA em novembro de 2022\footnote{\href{https://media.defense.gov/2022/Nov/10/2003112742/-1/-1/0/CSI_SOFTWARE_MEMORY_SAFETY.PDF}{Software Memory Safety | U/OO/219936-22}}, que recomendou que a linguagem fosse evitada.

Ademais, o módulo de RI [\ref{sub:ri}], escrito em Rust, demandava códigos \textit{wrappers} para que fosse possível utilizar a biblioteca em C++, dado que as linguagens não compartilham das mesmas estruturas de dados. Por exemplo, não é possível usar uma \textit{string} de C++ (\texttt{std::string}) em Rust, ou vice-versa (\texttt{\&str}/\texttt{String}).

Algumas abordagens podem ser usadas para circunventar o problema, mas cada uma trouxe sua própria série de complicações:

\begin{itemize}
    \item O projeto \href{https://github.com/mozilla/cbindgen}{cbindgen} provê um binário capaz de gerar cabeçalhos de C ou C++ a partir de arquivos fontes de Rust e um arquivo de configuração.
    
    Para que a geração dos cabeçalhos seja possível, macros especiais devem ser usados para declarar estruturas, enumeradores e funções com representações válidas na ABI (\textit{Application Binary Interface}) de C/C++.

    Apesar do valor da geração automática dos cabeçalhos, alguns problemas se destascam:

    \subitem Todas as funções que oferecem interoperabilidade entre as linguagens devem usar ponteiros para tipos abstratos. Isto significa abdicar da segurança provida pelo Rust, e re-introduz problemas relacionados ao gerenciamento de memória e ponteiros nulos;

    \subitem Ponteiros só podem ser liberados pela mesma linguagem que os alocou, uma vez que os modelos de alocação são diferentes entre as linguagens. Isto torna o problema de gerenciamento de memória manual ainda mais complicado do que o usual;

    \subitem Em versões modernas de C++, as regras de 3/5/0 causadas pelo RAII (\textit{Resource Acquisition Is Initialization}) muitas vezes tornam estruturas e enumeradores com tipos opacos impossíveis de serem construídos na linguagem.

    \item O projeto \href{https://github.com/rust-lang/rust-bindgen}{bindgen} Um projeto que almeija ser a contra-parte do cbindgen, gerando definições em Rust para estruturas de C/C++. Como a biblioteca de RI é escrita em Rust, isto significaria acoplá-la ao projeto da interface gráfica, conhecendo seus detalhes de implementação;
    
    \item \href{https://github.com/dtolnay/cxx}{cxx.rs}: TODO
\end{itemize}
