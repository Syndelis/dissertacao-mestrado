...
%importância da representação em árvore:
 ...
 % - Facilita a propagação de modificações; Exemplo: troca de sinal de um termo, remoção de um termo  (explicar embaixo)
 ...
 % - Permite que novos modelos sejam definidos utilizando a estrutura atual. 

% O código gerado para a exportação e simulação das EDOs baseia-se na substituição de \textit{strings} em \textit{templates}, dado que o software sempre armazena as EDOs do mesmo jeito e as EDOs em si possuem uma representação simples. O código de Python utiliza a biblioteca [[ NOME DA BIBLIOTECA ]] para simular o conjunto de equações e a biblioteca \textit{matplotlib} para plotar o resultado.
